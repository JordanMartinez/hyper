...
cookies :: forall m req (res :: ResponseState)
  .  IxMonad m
  => Request m
  => m (req & res) (req & res) (Either String (Object Values))
cookies = Ix.do
  { headers } <- getRequestData
  ipure (maybe (Right Object.empty) parseCookies (Object.lookup "cookie" headers))
...
setCookie :: forall m (req :: RequestState) b
  .  IxMonad m
  => Response m b
  => Name
  -> Value
